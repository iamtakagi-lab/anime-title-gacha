  ...
  return (
    <section id="container">
      {work && (
         ...
         className="btn"
         // TODO: "&"を含む文字列に対応させる
         onClick={() => open(`https://twitter.com/intent/tweet?text=アニメタイトルガチャで、${work ? work.title.replace("&", "＆") : ""}を引きました。ガチャを回した回数: ${count}回&url=https://anime-title-gacha.iamtakagi.net`, '_blank')}
       >
         🐥結果をTwitterでシェアしよう
       </button>
      )}
      <button className="btn" id="random_btn" onClick={async () => await pick()}>
          🔃ガチャを回してみる
      </button>
      {work && (
      <div id="work_card">
